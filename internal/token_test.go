...
func TestRetrieveToken_InParams(t *testing.T) {
	...
	defer ts.Close()
	_, err := RetrieveToken(context.Background(), context.Background(), clientID, "", ts.URL, url.Values{}, AuthStyleInParams)
	if err != nil {
...
func TestRetrieveTokenWithContexts(t *testing.T) {
	...
	_, err := RetrieveToken(context.Background(), context.Background(), "", "", ts.URL, url.Values{}, AuthStyleUnknown)
	if err != nil {
	...
	cancel()
	_, err = RetrieveToken(ctx, context.Background(), "", "", cancellingts.URL, url.Values{}, AuthStyleUnknown)
	close(retrieved)
	if err == nil {
